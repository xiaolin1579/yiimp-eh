...
function doCrex24Trading($quick=false)
{
	...
	foreach ($data as $balance)
	{
        ...
        $db_orders_count = dboscalar("select COUNT(uuid) from orders where market='{$exchange}'");
        $db_orders_exist = ($db_orders_count ? $db_orders_count>0 : false);        
        //debuglog("{$exchange} orders count {$db_orders_count}, condition={$db_orders_exist}");        
		if (($balance->available+$balance->reserved)==0 && !$db_orders_exist) continue;
		...
		if(is_array($orders) && !empty($orders))
        {
            foreach($orders as $order)
            {
                ...
                {
                    debuglog("{$exchange}: cancel order $pair at $sellprice, ask price is now $ask");
                    sleep(1);
        ...
        debuglog("{$exchange}: selling params ".json_encode($params));
		$res = crex24_api_user('trading/placeOrder', '', json_encode($params));
		if(!$res) {
			debuglog("{$exchange} SubmitTrade err: ".json_encode($res));
			continue;
	...
	$withdraw_fee = exchange_get($exchange, 'withdraw_fee_btc', 0.0011);
	...
	if(floatval($withdraw_min) > 0 && $savebalance->balance >= ($withdraw_min + $withdraw_fee))
	{
        ...
        debuglog("{$exchange} withdraw result:".json_encode($res));
		if(is_object($res) && $res->Success)
		{
			...
			debuglog("{$exchange} withdraw BTC error: ".json_encode($res));
		}
	}
